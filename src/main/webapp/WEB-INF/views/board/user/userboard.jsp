...
<body>
					...
					<div class="col-lg-8 mb-5 mb-lg-0">
						<div class="blog_left_sidebar">
							...
							<button id="wirteBtn"
								...
								type="button" style="color: #eb566c !important">글 등록</button>			
							</c:if>							
							...
							<c:if test="${list.size()!=0}">
								<c:forEach var="board" items="${list}">
									<c:choose>
										<c:when test="${board.importance>0}">
											<article id="importance1" class="board"
												style="background-color: #eb566c; color: white;">
												<div class="blog_details">
													<a class="d-inline-block"
														href="${root}/board/user/select?no=${board.no}">
														<h2>${board.title}</h2>
													</a>
													<p>${board.content}</p>
					...
					<div class="col-lg-4">
						<div class="blog_right_sidebar">
							...
							<aside class="single_sidebar_widget search_widget">
								<form action="${root}/board/user/list">
									<div class="form-group">
										<div class="input-group mb-3">
											<input type="text" class="form-control" name="searchWord"
												id="searchWord" placeholder="Search Keyword"
												onfocus="this.placeholder = ''"
												onblur="this.placeholder = 'Search Keyword'" />
											<div class="input-group-append">
												<button class="btns" type="button" id=serchBtn>
													<i class="ti-search"></i>
												</button>
											</div>
										</div>
								...
								<a href="${root}/board/user/list" style="color: black">
									<button
										class="button rounded-0 primary-bg text-white w-100 btn_1 boxed-btn"
										type="submit">전체글 보기</button>
								</a>
							</aside>
	...
	<script>		
		  $("#wirteBtn").click(()=>{		    	 
		    	  location.href = "/carrotmap/board/user/insert";
		  });
	</script>
